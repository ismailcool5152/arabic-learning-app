    ...
    if (typeof parsed === "object" && parsed !== null) {
      ...
      if (parsed.quranic_ai_translations || parsed.offline_saved_verses || parsed.quranic_arabic_master_flashcards || parsed.quranic_arabic_saved_maps) {
        ...
        if (parsed.quranic_arabic_master_flashcards && Array.isArray(parsed.quranic_arabic_master_flashcards)) {
            const raw = localStorage.getItem('quranic_arabic_master_flashcards');
            const existing = raw ? JSON.parse(raw) : [];
            const newSet = new Set([...existing, ...parsed.quranic_arabic_master_flashcards]);
            localStorage.setItem('quranic_arabic_master_flashcards', JSON.stringify(Array.from(newSet)));
        }
        ...
        if (parsed.quranic_arabic_reviewed_flashcards && Array.isArray(parsed.quranic_arabic_reviewed_flashcards)) {
            const raw = localStorage.getItem('quranic_arabic_reviewed_flashcards');
            const existing = raw ? JSON.parse(raw) : [];
            const newSet = new Set([...existing, ...parsed.quranic_arabic_reviewed_flashcards]);
            localStorage.setItem('quranic_arabic_reviewed_flashcards', JSON.stringify(Array.from(newSet)));
        }
        ...
        if (parsed.quranic_arabic_saved_maps && Array.isArray(parsed.quranic_arabic_saved_maps)) {
            const raw = localStorage.getItem('quranic_arabic_saved_maps');
            const existing = raw ? JSON.parse(raw) : [];
            const existingIds = new Set(existing.map((m: any) => m.id));
            const toAdd = parsed.quranic_arabic_saved_maps.filter((m: any) => m.id && !existingIds.has(m.id));
            const merged = [...existing, ...toAdd];
            localStorage.setItem('quranic_arabic_saved_maps', JSON.stringify(merged));
        }
        ...
        if (parsed.quranic_arabic_recent_searches && Array.isArray(parsed.quranic_arabic_recent_searches)) {
            const raw = localStorage.getItem('quranic_arabic_recent_searches');
            const existing = raw ? JSON.parse(raw) : [];
      ...
      // Dispatch an event to notify React components to reload local storage data
